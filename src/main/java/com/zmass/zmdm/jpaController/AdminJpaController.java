...
public class AdminJpaController implements Serializable {
    ...
    public Admin findUser(Admin admin){
        EntityManager em = getEntityManager();
        try {
//            System.out.println("email: "+admin.getEmail()+admin.getPassword());
           Admin user = (Admin) em.createQuery("select a from Admin a where a.password='"+admin.getPassword()+"' and a.email='"+admin.getEmail()+"'").getSingleResult();
           return user;
        }
        catch(Exception e){
            e.printStackTrace();
